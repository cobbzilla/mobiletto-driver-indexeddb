...
exports.storageClient = exports.StorageClient = exports.IdbInfo = void 0;
...
class StorageClient {
    constructor(dbName, opts) {
        ...
        this.info = () => (Object.assign({ canonicalName: () => `indexeddb:${this.dbName}` }, exports.IdbInfo));
        this.mdb = () => __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.db)
                return this.db;
            throw new mobiletto_base_1.MobilettoError(`mdb: error getting database`);
        });
        if (!dbName) {
            throw new mobiletto_base_1.MobilettoError("indexeddb.StorageClient: key (dbName) is required");
        }
        this.dbName = dbName;
        if (opts && opts.indexedDB) {
            ...
            throw new mobiletto_base_1.MobilettoError("indexeddb.StorageClient: opts.indexedDB is required");
        }
        this.dbPromise = new Promise((resolve, reject) => {
            ...
            openOrCreateDB.onupgradeneeded = (event) => {
                ...
                if (!req.result) {
                    reject(new mobiletto_base_1.MobilettoError("indexedDB.upgradeneeded: event.target.result was not found"));
                    return;
                }
                db = req.result;
                db.onerror = () => {
                    reject(new mobiletto_base_1.MobilettoError("indexedDB.open failed (upgradeneeded)"));
                };
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    listTx.oncomplete = () => {
                        mobiletto_base_1.logger.info(`list(${pth}) listTx completed`);
                    };
                    listTx.onerror = () => {
                        mobiletto_base_1.logger.error(`list(${pth}) listTx error`);
                    };
                    ...
                    listRequest.onsuccess = () => __awaiter(this, void 0, void 0, function* () {
                        ...
                        if (foundExact && !recursive) {
                            ...
                            if (visitor) {
                                for (const f of filtered.filter((o) => o.type === mobiletto_base_1.M_FILE)) {
                                    yield visitor(f);
                                }
                            }
                            resolve(filtered);
                    ...
                    mobiletto_base_1.logger.error(`list(${pth}): error ${e}`);
                }
            });
        });
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return new Promise((resolve, reject) => {
                ...
                readMetaTx.oncomplete = () => {
                    if (!data) {
                        reject(new mobiletto_base_1.MobilettoNotFoundError(path));
                        return;
                    }
                    try {
                        delete data.bytes;
                        ...
                        reject(new mobiletto_base_1.MobilettoError(`metadata(${path}) error parsing data: ${e}`));
                    }
                };
                readMetaTx.onerror = () => {
                    reject(new mobiletto_base_1.MobilettoError(`metadata(${path}) readMetaTx error`));
                };
                const readRequest = readMetaTx.objectStore(ROOT_STORE).get(path);
                readRequest.onerror = () => {
                    reject(new mobiletto_base_1.MobilettoError(`metadata(${path}) readRequest error`));
                };
    ...
    write(path, generatorOrReadableStream) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                ...
                if ((0, mobiletto_base_1.isReadable)(generatorOrReadableStream)) {
                    ...
                    let chunk = (0, mobiletto_base_1.isAsyncGenerator)(generator) ? (yield generator.next()).value : generator.next().value;
                    let nullCount = 0;
                    while (chunk || nullCount < 5) {
                        ...
                        chunk = (0, mobiletto_base_1.isAsyncGenerator)(generator) ? (yield generator.next()).value : generator.next().value;
                    }
                }
                const writeTx = db.transaction(ROOT_STORE, "readwrite");
                writeTx.oncomplete = () => {
                    mobiletto_base_1.logger.info(`write(${path}) writeTx complete`);
                    resolve(bytes.length);
                };
                writeTx.onerror = () => {
                    reject(new mobiletto_base_1.MobilettoError(`write(${path}) writeTx error`));
                };
                const writeObject = {
                    name: path,
                    type: mobiletto_base_1.M_FILE,
                    bytes,
                    size: bytes.length,
                    mtime: Date.now(),
                };
                ...
                writeRequest.onerror = () => {
                    reject(new mobiletto_base_1.MobilettoError(`write(${path}) writeRequest error`));
                };
                writeRequest.onsuccess = () => {
                    mobiletto_base_1.logger.info(`write(${path}) writeRequest success`);
                    resolve(bytes.length);
                };
            }));
        });
            ...
            return new Promise((resolve, reject) => {
                ...
                readTx.oncomplete = () => {
                    if (bytesRead) {
                        ...
                        mobiletto_base_1.logger.warn(`read(${path}) readTx complete but neither bytesRead nor notFound was set`);
                    }
                };
                readTx.onerror = () => {
                    reject(new mobiletto_base_1.MobilettoError(`read(${path}) readTx error`));
                };
                const readRequest = readTx.objectStore(ROOT_STORE).get(path);
                readRequest.onerror = () => {
                    reject(new mobiletto_base_1.MobilettoError(`read(${path}) readRequest error`));
                };
                readRequest.onsuccess = () => {
                    if (typeof readRequest.result === "undefined") {
                        notFound = new mobiletto_base_1.MobilettoNotFoundError(path);
                        return;
                    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const deleteVisitor = (file) => __awaiter(this, void 0, void 0, function* () {
                return yield new Promise((resolve, reject) => {
                    ...
                    if (file.type !== mobiletto_base_1.M_FILE) {
                        mobiletto_base_1.logger.warn(`remove(${path}): not a file`);
                        if (quiet) {
                            resolve(true);
                        }
                        else {
                            const err = new mobiletto_base_1.MobilettoError(`remove(${path}): not a file`);
                    ...
                    deleteTx.oncomplete = () => {
                        if (deleteResult == null) {
                            if (quiet) {
                                mobiletto_base_1.logger.warn(`remove(${path}): deleteResult was null`);
                                resolve(true);
                            }
                            else {
                                const err = new mobiletto_base_1.MobilettoError(`remove(${path}): deleteResult was null`);
                                deleteErrors.push(err);
                                reject(err);
                            }
                        }
                        else {
                            if (deleteResult instanceof mobiletto_base_1.MobilettoNotFoundError || deleteResult instanceof mobiletto_base_1.MobilettoError) {
                                if (quiet) {
                                    resolve(true);
                                }
                                else {
                    ...
                    deleteTx.onerror = () => {
                        if (quiet) {
                            mobiletto_base_1.logger.warn(`remove(${path}): deleteTx error`);
                            resolve(true);
                        }
                        else {
                            const err = new mobiletto_base_1.MobilettoError(`remove(${path}) deleteTx error`);
                            deleteErrors.push(err);
                            reject(err);
                        }
                    };
                    ...
                    findRequest.onerror = () => {
                        mobiletto_base_1.logger.warn(`remove(${path}): error`);
                        deleteResult = new mobiletto_base_1.MobilettoNotFoundError(path);
                    };
                    findRequest.onsuccess = () => {
                        if (typeof findRequest.result === "undefined") {
                            ...
                            deleteRequest.onerror = () => {
                                deleteResult = new mobiletto_base_1.MobilettoError(`remove(${path}) deleteRequest error`);
                            };
                        }
                    };
                });
...
const storageClient = (key, secret, opts) => {
    if (!key) {
        throw new mobiletto_base_1.MobilettoError("indexeddb.storageClient: key is required");
    }
    if (!opts || !opts.indexedDB) {
        throw new mobiletto_base_1.MobilettoError("indexeddb.storageClient: opts.indexedDB is required");
    }
